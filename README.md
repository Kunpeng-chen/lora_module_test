...
The `error_at` suite expands `ERRAT-001` through `ERRAT-057` from `lora_auto/config/formal/error_at_cases.yaml`. All abnormal AT cases expect `ERROR=101`. Receiving the configured error code is a PASS condition for the negative command step. Each negative command is followed by a post-check `AT -> OK`; missing the expected error code or failing the post-check makes the case FAIL.
